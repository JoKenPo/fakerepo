...
const jwt_pwd: string = (process.env.JWT_SECRET || '$F5u@y7*!a').substring(0, 10);

const jwtHash = (key: string): string => new CipherService().Hash(key, jwt_pwd + salt + md5(key).substring(0, 12), EHashAlgorithm.sha256);
