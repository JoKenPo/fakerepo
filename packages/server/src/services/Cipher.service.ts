...
export default class CipherService {
    ...
    public Hash(data: string, secret: string, algorithm: EHashAlgorithm = EHashAlgorithm.sha512) {
        return crypto.createHmac(algorithm, Right(salt + secret, 32))
            .update(data, 'utf8')
            .digest('hex')
    }

    public Encrypt(data: any, secret: string, algorithm: ECipherAlgorithm = ECipherAlgorithm.aes256cfb): string {
        const iv = crypto.randomBytes(16);
        const cipher = crypto.createCipheriv(algorithm, Right(salt + secret, 32), iv);
        const encrypted = Buffer.concat([cipher.update(JSON.stringify(data)), cipher.final()]);
        const res = JSON.stringify({ iv: iv.toString('hex'), content: encrypted.toString('hex') });
        return this.toBase64(res);
    }

    public Decrypt(data: string, secret: string, algorithm: ECipherAlgorithm = ECipherAlgorithm.aes256cfb): any {
        const hash = JSON.parse(this.fromBase64(data));
        const decipher = crypto.createDecipheriv(algorithm, Right(salt + secret, 32), Buffer.from(hash.iv, 'hex'));
